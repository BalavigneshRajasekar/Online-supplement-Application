...
function SingleViewOrder() {
  ...
  const changeOrderStatus = async () => {
    ...
    let index = orderStatus.findIndex(
      (value) => value === singleOrder.orderStatus
    );
    console.log(index + 1);
      ...
      {singleOrder ? (
        <Box sx={{ marginTop: 1 }}>
          <div className="d-flex justify-between">
            ...
            >
              <input type="radio" checked></input> {singleOrder.orderStatus}
            </Tag>
          </div>
          <div
            className="p-3 d-flex flex-wrap gap-7 spanStyle"
                  ...
                  <List.Item.Meta
                    ...
                    description={
                      <p>
                        ...
                        />
                        {item.price} <b>Quantity:</b> {item.quantity}
                      </p>
                    }
                  />
                  <b>
          ...
          <div className="d-flex flex-column flex-md-row gap-4 p-3 justify-around">
            ...
            <div className="d-flex flex-md-column justify-center  gap-5">
              ...
              {singleOrder.orderStatus == orderStatus[0] && (
                <button
                  disabled={singleOrder.orderStatus == "Delivered"}
                  className="border p-3 rounded-md  bg-black text-white active:scale-95"
              ...
              {singleOrder.orderStatus == orderStatus[1] && (
                <button
                  disabled={singleOrder.orderStatus == "Delivered"}
                  className="border p-3 rounded-md  bg-orange-700 text-white active:scale-95"
              ...
              {singleOrder.orderStatus == orderStatus[2] && (
                <button
                  disabled={singleOrder.orderStatus == "Delivered"}
                  className="border p-3 rounded-md  bg-yellow-300 text-white active:scale-95"
                  onClick={() => changeOrderStatus()}
                >
              ...
              {singleOrder.orderStatus == orderStatus[3] && (
                <button
                  disabled={singleOrder.orderStatus == "Delivered"}
                  className="border p-3 rounded-md  bg-green-600 text-white active:scale-95"
                  onClick={() => changeOrderStatus()}
                >
